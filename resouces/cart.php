...
// add
    if(isset($_GET['add']))
    {
        ...
        while($row = fetch_array($query))
        {
            ...
            {
                set_message("We only heve ". $row['product_quantity']." "."{$row['product_title']}"." available");
                redirect("../public/checkout.php");
            }
        }
                ...
                while($row = fetch_array($query))
                {
                    ...
                    <tr>
                        <td>{$row['product_title']}<br/>
                            <img src="../resources/uploads/{$row['product_image']}" height="80" width="150" >
                        </td>
                        <td> $prod_price RSD</td>
                        <td>{$value}</td>
                        <td>{$sub} RSD</td>
                        <td>
...
DELIMETER;
                // {$value} umesto  {$row['product_quantity']}
        ...
        {   
            if($value > 0)
            {
                if(substr($name, 0, 8) == "product_")
                {
                    ...
                    //  za test -  http://localhost/ecom/public/thank_you.php?tx=3517888&amt=350&cc=RSD&st=Completed
                    $send_order = query("INSERT INTO orders (order_amount, order_transaction, order_status, order_currency)
                                            VALUES ('{$amount}', '{$transaction}', '{$status}', '{$currency}') RETURNING order_id");
                    $last_id_arr = fetch_array($send_order);
                    $last_id = $last_id_arr['order_id'];
                    // echo $last_id;
                    confirm($send_order);
                    ...
                    {   
                        ...
                        // $product_quantity = $row['product_quantity']; // imam value tj quantity za svaku

                        $sub = $row['product_price']*$value;
                        $item_quantity += $value;

                        $insert_report = query("INSERT INTO reports (product_id, report_price, report_quantity, order_id, product_title)
